...
prompt = ChatPromptTemplate.from_messages([
    ("system", """
        ...
        Your job is to generate a {length} Detailed script for a summary video based on news article transcripts on the topic: {topic}. 
        ...
        {{
        "video_title": "[A concise, catchy, and relevant video title based on the topic and script]",
        "description": "[A clear and informative video description related to the video content and script]",
        "tags": ["[tag1]", "[tag2]", "[tag3]", "..."],

        "script": "[Your full script goes here]",
        "mood": "[Your mood description goes here, e.g., 'Speak in a Professional and sad tone']"
        }}

        Guidelines:
        - Begin the script with today's date in this format: "It is [todaysdate] and you are watching FactLine." Replace [todaysdate] with {date} in plain English.
        ...
        - End the script with a sign-off like: "Thanks for watching FactLine. Stay informed and see you next time."

        Important:
        - Your output should follow the JSON structure exactly.
...
if st.button("🚀 Submit"):
    if not user_input:
        ...
        with st.spinner("⏳ Hold On, Your Video is being uploaded..."):
            ...
            audio_name = make_audio(data)
            video_name = generate_fullwidth_waveform_video(audio_file=audio_name, logo_file=logo)

            # Upload the video
            upload(data=data, video_file=video_name)
